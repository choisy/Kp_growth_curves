...
```{r}
sample_medium <- od_data |> 
  filter(between(OD, .1, .8), time < 15 * 60) |> 
  group_by(Sample, Medium) |> 
  group_modify(~ tibble(fitted = list(calibrate(.x, logistic, normal))))
```
...
f <- function(x, i) {
  pnames <- c("K", "r")
  bnames <- c("_lwr", "_upr")
  setNames(x[pnames, i], paste0(pnames, bnames[i]))
}
```

```{r eval = FALSE}
sample_medium_ci <- sample_medium |> 
  mutate(coef  = map(fitted, coef2),
         CI    = map(fitted, confint2),
         lower = map(CI, f, 1),
         upper = map(CI, f, 2)) |> 
  unnest_wider(coef) |> 
  unnest_wider(lower) |> 
...
```{r include = FALSE}
if (file_exists("sample_medium_ci.rds")) {
  sample_medium_ci <- readRDScache("sample_medium_ci.rds")
} else {
  sample_medium_ci <- sample_medium |> 
    mutate(coef  = map(fitted, coef2),
           CI    = map(fitted, confint2),
    ...
    unnest_wider(upper) |> 
    select(-t0, -k)
  saveRDScache(sample_medium_ci, "sample_medium_ci.rds")
}
```

```{r plot_comparisons_r}
strains <- unique(sample_medium_ci$Sample)
x1 <- setNames(seq_along(strains), strains)
media <- unique(sample_medium_ci$Medium)
x2 <- setNames((seq_along(media) - 1.5) / 4, media)
cols <- setNames(c(col1, col2), media)

with(sample_medium_ci, {
  plot(x1[Sample] + x2[Medium], r, col = cols[Medium], pch = 19,
       ylim = range(c(r_lwr, r_upr)),
       xlab = NA, ylab = "max growth rate", axes = FALSE)
  axis(1, x1, strains)
  axis(2)
  arrows(x1[Sample] + x2[Medium], r_lwr, x1[Sample] + x2[Medium], r_upr, .05, 90, 3,
         col = cols[Medium], lwd = 2)
})
```
